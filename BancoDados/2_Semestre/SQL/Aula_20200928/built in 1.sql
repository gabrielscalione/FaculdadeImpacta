...
SELECT EMAIL,LEFT(EMAIL,5) [LEFT] FROM PESSOA 
SELECT EMAIL,RIGHT(EMAIL,5) [RIGHT] FROM PESSOA 
SELECT EMAIL,LEFT(TELEFONE,2) DDI FROM PESSOA
...
SELECT CPF, REVERSE(CPF) REVERSE FROM PESSOA

--CONCAT - CONCATENA
SELECT CONCAT('O CPF DO CLIENTE ',NOME,' É ', CPF) CONCATENAÇÃO FROM PESSOA

--REPLACE - SUBSTITUI UMA PARTE DO TEXTO POR OUTRA
SELECT EMAIL,
REPLACE(EMAIL,'.com','.com.br'),
NOME,
REPLACE(NOME,'A','@') NOME_ARROBA,
REPLACE(NOME,'O','0') NOME_ZERO,
REPLACE(REPLACE(NOME,'O','0'),'A','@') NOME_ZERO_ARROBA
FROM PESSOA
...
CHARINDEX(' ',TRIM(NOME)) BUSCA_PRIMEIRO_REAL,  
...
DATEDIFF(YEAR,DATA_NASCIMENTO,'2001-01-11') IDADE_11_SETEMBRO  --DIFERENCA DE DATAS,ENTRE NASCIMENTO E DATA DE HOJE
FROM PESSOA
...
CAST (DATA_NASCIMENTO AS VARCHAR(100)) TELEFONE_CAST,
...
SELECT CEILING(SALARIO)[CEILING] FROM PESSOA
...
SELECT RAND() RANDOMICO,SALARIO,ROUND(SALARIO,2)[ROUND] FROM PESSOA
