...
-- Aula 02 - Entendimento de um banco de dados, com variáveis.
------------------------------------------------------
...
------------------------------------------------------
- Repare que até agora, com variáveis ou não, o enunciado deve ser
entendido da mesma forma, você poderá observar que 
acrescentar as variáveis apenas ajuda no processo como um todo.
...
------------------------------------------------------
- Aqui as primeiras mudanças, para coletar o ID
da localidade, eu preciso de um local para armazená-lo.
E se você pensou em variáveis, está no caminho certo.
...
-Repare que eu não só devolvi ID para a 'tela'
eu o armazenei para uso futuro, na memória da variável.

-- #2 -- veículo
- Declaro um lugar para guardar o veículo
...
-- #3 -- plano
Declaro um lugar para guardar o plano
Declare @idPlano INT
...
- Agora, para cada select, eu posso realizar uma coleta
	Só preciso de variáveis para coletar os valores
DECLARE @Atual INT
		, @total INT
...
#2	/* Se o total de veículos for inferior à capacidade, registrar o estacionamento*/
	select @total = capacidade_moto from localidade where identificacao = 'Faculdade Impacta - Paulista'	
- Este é simples, já que a capacidade do estacionamento
por tipo de veículo é um dado fornecido e já salvo na tabela

-Repare que eu não só devolvi os valores para a 'Tela'
eu o armazenei para uso futuro, na memória da variável.
...
-- Não preciso mais usar valores fixos
-- VALUES ( 1, 11, 6 ) 

-- posso usar os valores previamente coletado nas variáveis.
VALUES ( @idLocalidade, @idVeiculo, @idPlano ) 

-Finalmente, posso coletar o ticket e utilizá-lo
DECLARE @ticket INT -- ticket é o id do estacionamento

/*descubro o último ID utilizado para devolve-lo.*/
select @ticket = @@identity
...
--Não preciso mais usar um ID fixo no código, posso usar a variável.
--WHERE	estacionamento.id = 13 /*suposto Ticket do estacionamento*/
WHERE	estacionamento.id = @ticket
