...
02) Sejam as seguintes afirmações sobre Isolamento entre Processos.
		...
		IV		Uma transação pode conter inúmeras instruções, porém, os únicos 
				estados aceitáveis, considerados consistentes, são aqueles em que
				ou todas as instruções rodaram, ou nenhuma instrução rodou.
...
03) Sejam as seguintes afirmações sobre Erros e tratamento de erros.
		...
		III		Erros customizados ou personalizados podem ser criados para
		...
		V		Um dos maiores benefícios do tratamento de erros é, além da devida 
				...
				sobre processos de importação, que muitas vezes trazem exceções
...
05) Sejam as seguintes afirmações sobre procedimentos armazenados em MSSQL.
		...
		II		Assim como na instrução INSERT, onde a cláusula OUTPUT serve para 
...
DECLARE @CPF CHAR(14) = dbo.fn_limpaCPF('12345678909')
select dbo.fn_calculaPrimeiroDigito(@CPF)

O que há de errado com essa função ?
*A) Não há nada de errado com esta função, ela devolve o que é esperado. --Também aceita como correta
...
GO
DECLARE @CPF CHAR(14) = dbo.fn_limpaCPF('123.456.789-09')
select dbo.fn_calculaSegundoDigito(@CPF)
...
--Cabeçalho extraído da questão 12
...
END
GO
--Uso da procedure extraído da Questão 13
DECLARE @CPF VARCHAR(255) = '529.982.247-25', @retorno VARCHAR(255)
EXEC pr_validaCPF @CPF, @retorno OUTPUT
SELECT 'O CPF : '+ @CPF + ' é ' + @retorno
--Retorno esperado: O CPF : 529.982.247-25 é VÁLIDO
